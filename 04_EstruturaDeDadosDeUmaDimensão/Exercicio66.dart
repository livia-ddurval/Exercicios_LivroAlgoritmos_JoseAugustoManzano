...
// Descrição:
// Este exercício consiste em desenvolver um programa que leia dois vetores com 15 elementos inteiros quaisquer e construa um 
//terceiro vetor que será a junção dos dois primeiros vetores, ou seja, terá 30 elementos.
...
void main() {
  List<int> vetor1 = [];
  for (int i = 0; i < 15; i++) {
    stdout.write("Elemento vetor1 ${i + 1}: ");
    int elemento = int.parse(stdin.readLineSync()!);
    vetor1.add(elemento);
  }
  List<int> vetor2 = [];
  for (int i = 0; i < 15; i++) {
    stdout.write("Elemento vetor2 ${i + 1}: ");
    int elemento = int.parse(stdin.readLineSync()!);
    vetor2.add(elemento);
  }
  List<int> vetor3 = [];
